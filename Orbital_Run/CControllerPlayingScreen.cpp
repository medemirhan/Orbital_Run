...
INT32S CControllerPlayingScreen::InvokeScreen(sf::RenderWindow& app, CConfigurationData& config_data, CModel& model)
{
	...
	while (app.isOpen()) {
		...
		while (app.pollEvent(event)) {
			...
			//auto future = std::async(&CController::EventHandlerPlayingScreen, controller, event, game_state, *this, config_data, &clock_rocketright, &clock_bomb_addition, &clock_bomb_removal, &num_orbits);
			//return_val = future.get();
			if (change_screen) {
				return return_val;
			}
		}
